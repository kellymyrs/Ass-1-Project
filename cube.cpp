...
int main(int argc,char* argv[]){
	...
	int i,j,k = 3 ,M = 10, N,d,probes = 2; // N = number of items(lines) in the file, d = number of coordinates (dimension)
	char* com;
	...
	for( i = 1 ; i < argc - 1 ; i++) {
		...
		if(!strcmp(argv[i],"-Μ"))      //Μ = max number of candidate points 
			M = atoi(argv[i+1]);


        if(!strcmp(argv[i],"-probes"))      //probes = max number of vertexes 
    ...
    //Create Hashtable
    unordered_map<int,vector <Item<int>*>> hashtable;    
	...
	if(!query_file.is_open()){
		...
		cin >> com;           //get the query file
		query_file.open(com); //open the query file
	}
    ...
    while(getline(new_q_file,line)) {
        ...
        if( R != 0 ){
           Range(c,hashtable,metric,R,M,probes,concat,range);
        }
        ...
        min_dist = 10000000;
        nn = NN(c,hashtable,metric,M,probes,concat,min_dist);               
